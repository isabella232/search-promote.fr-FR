...
## Utilisation de règles de classement {#concept_F555C076759B4E81B925441CFE707397}
...
Voir [Ajouter un groupe](../c-about-rules-menu/c-about-ranking-rules.md#task_B65081B3CC9E4330A7FEE77B7BCD36C8)de règles de classement.
...
1. Choisissez parmi les options suivantes :
   ...
   <table> 
    ...
    <tbody> 
      ...
      <li id="li_2A839874D71D45FEA661B3D3B8BE2A86"> Sur la page Ajouter un champ, dans le champ de <span class="uicontrol"> texte Nom du champ, tapez </span> <code>
        rank 
      </code>; dans le champ de texte <span class="uicontrol"> Meta Tag Name </span> , saisissez <code>
        rank 
      </code>; dans la liste <span class="uicontrol"> déroulante Type de données </span> , sélectionnez <span class="uicontrol"> Classement </span>. Conservez toutes les autres options de champ en l’état. <p>Reportez-vous au paramètre de requête <span class="codeph"> sp_sr </span> dans les paramètres CGI de recherche <a href="../c-appendices/c-cgiparameters.md#reference_582E85C3886740C98FE88CA9DF7918E8" type="reference" format="dita" scope="local"> principal </a>. </p> </li> 
...
1. ajoutez une ou plusieurs règles de classement.
   ...
   Voir [Ajouter une règle](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)de classement.
...
1. Vérifiez les valeurs de la colonne Classement dans **[!UICONTROL Settings]** > **[!UICONTROL Metadata]** > **[!UICONTROL Definitions]** pour vérifier que vos règles de classement ont été appliquées correctement.
...
* La fonction intégrée spéciale, `search_get_age_rank()`spécifiée dans les pages Ajouter ou Modifier la règle de classement, est utilisée pour calculer le classement d’un document par âge. Les sections suivantes décrivent en détail l&#39;utilisation générale de la fonction de classement par âge. Vous trouverez également un exemple illustrant la fonction de classement par âge.
...
la date contenue dans le champ du document `birthdate` est transmise en tant qu’horodatage. La demi-vie est de 28 jours. La valeur de classement par défaut est 0,20 si la date n’est pas valide.

Voir le tableau des options dans [Ajouter une règle](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)de classement.

Dans la section Valeurs/Classements de la page Ajouter une règle de classement ou Modifier une règle de classement, vous ne pouvez utiliser que `search_get_age_rank` des règles personnalisées. Par conséquent, veillez à choisir **Personnalisé** dans la liste déroulante Valeurs/Classements. Lorsque la règle utilise la fonction de classement par âge, aucun espace n’est autorisé dans la partie valeurs de la règle. Assurez-vous qu&#39;il n&#39;y a pas d&#39;espaces entre les arguments de fonction ou entre eux. Et il n&#39;y a pas d&#39;espaces entre des opérateurs mathématiques ou conditionnels.
...
Au fur et à mesure que chaque document est traité, la valeur en `other_field` est convertie au format &quot;époque&quot; Unix, en utilisant les définitions de format de date du champ. Cette valeur est utilisée dans l’ `search_get_age_rank()` appel. Comme chaque valeur &quot;époque&quot; est inférieure à 999999999999 (du moins pour l’instant), la règle fournit simplement la valeur de retour de la fonction (de 0 à 1) comme rang.
...
Voir [Ajouter un nouveau champ](../c-about-settings-menu/c-about-metadata-menu.md#task_6DF188C0FC7F4831A4444CA9AFA615E5)de balise meta.
...
Utilisez la fonction Vue de données pour afficher rapidement les résultats de la fonction de classement par âge. ajoutez les champs de métadonnées appropriés. Dans l’exemple, `age_val` et `myrank` sont les deux champs de métadonnées qui doivent être ajoutés à la Vue de données. Le `myrank` champ montre comment le classement par âge affecte les valeurs de classement. Le `age_val` champ affiche la sortie brute de la fonction de désintégration exponentielle pour ce document.
...
* Valeur par défaut que vous pouvez saisir dans la `search_get_age_rank()` fonction elle-même.
...
1. (Facultatif) Si vous avez créé un groupe de règles et ajouté des règles au groupe, sur la [!DNL Define Ranking Rules] page, dans la liste **[!UICONTROL Select Rule Group]** déroulante, sélectionnez un groupe de règles contenant les règles que vous souhaitez modifier.

   Voir [Ajouter un groupe](../c-about-rules-menu/c-about-ranking-rules.md#task_B65081B3CC9E4330A7FEE77B7BCD36C8)de règles de classement.
1. Sur la [!DNL Define Ranking Rules] page, cliquez sur **[!UICONTROL Add Rule]** pour ajouter une nouvelle règle de classement ou pour ajouter une référence à un jeu de règles.
1. Sur la [!DNL Add Ranking Rule] page, définissez les options de votre choix. Les champs signalés par un astérisque (*) sont obligatoires.
   ...
   Le type de source de données que vous sélectionnez affecte les choix disponibles dans la liste [!DNL Data Source Name] déroulante. Par exemple, si vous avez sélectionné **[!UICONTROL Meta Tag]** le type de source de données, le nom de la source de données fait référence au nom d’une balise meta sur les pages de votre site Web. Si vous avez sélectionné **[!UICONTROL Adobe Analytics Metric (Number)]**, le nom de la source de données fait référence à l’un des noms de mesure Adobe Analytics que vous avez sélectionnés dans une suite de rapports, comme indiqué sur la **[!UICONTROL Edit Adobe Analytics Metrics]** page de la recherche/marchandisage sur le site.
   ...
   <table> 
    ...
    <tbody> 
      ...
      <li id="li_457DE44D6ADA40619DC77220BF12318E"> <span class="uicontrol"> Personnalisé </span> <p>Spécifie des paramètres personnalisés. Par exemple, une source de données nommée "marque" peut contenir le nom de marque d’un produit particulier. Vous pouvez indiquer l’importance relative de chaque marque en l’répertoriant avec son classement. </p> </li> 
        ...
        regexp 
      </code>. </p> <p>Voir Expressions <a href="../c-appendices/r-regular-expressions.md#reference_B5BA7D61D82E4109A01D2A2D964E3A6A" type="reference" format="dita" scope="local"> régulières </a>. </p> <p>Dans l’exemple suivant, un document de résultats de recherche contenant le contenu de la marque "jeans Levis" est affecté d’un rang de 0,1. Comme pour la comparaison standard, la casse est ignorée pour les expressions régulières. <code> DKNY&nbsp;0.5 regexp&nbsp;Levis.*&nbsp;0.1 Lee&nbsp;0.2 </code> </p> </td> 
      </tr> 
      <tr> 
...
1. Dans le menu produit, cliquez sur **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**.
1. (Facultatif) Si vous avez créé un groupe de règles et ajouté des règles au groupe, sur la **[!UICONTROL Define Ranking Rules]** page, dans la liste **[!UICONTROL Select Rule Group]** déroulante, sélectionnez un groupe contenant les règles à modifier.

   Voir [Ajouter un groupe](../c-about-rules-menu/c-about-ranking-rules.md#task_B65081B3CC9E4330A7FEE77B7BCD36C8)de règles de classement.
1. Dans le tableau, sous l’en-tête de **[!UICONTROL Actions]** colonne, cliquez **[!UICONTROL Edit]** pour le nom de la source de données à modifier.
1. Sur la [!DNL Edit Ranking Rule] page, définissez les options de votre choix. Les champs signalés par un astérisque (*) sont obligatoires.
   ...
   Consultez le tableau des options sous [Ajouter une règle](../c-about-rules-menu/c-about-ranking-rules.md#task_A132789FD4E5423DAD090DCDA7311E8A)de classement.
...
1. Dans le tableau, sous l’en-tête de **[!UICONTROL Actions]** colonne, cliquez **[!UICONTROL Delete]** pour le nom de la source de données à modifier.
...
1. Sur la [!DNL Add Ranking Rule Group] page, dans le **[!UICONTROL Rule Group Name]** champ, tapez un nom unique pour le nouveau groupe de règles.
1. Dans la liste **[!UICONTROL Rank Field Name]** déroulante, sélectionnez le nom du champ de métadonnées de classement à associer au nouveau groupe de règles. Sélectionnez **[!UICONTROL None]** si vous ne souhaitez pas affecter de rang.
   ...
   La liste des noms de champ de classement provient des définitions de métadonnées qui ont été ajoutées dans **[!UICONTROL Settings]** > **[!UICONTROL Metadata]** > **[!UICONTROL Definitions]**.

   Consultez le tableau des options dans [Ajouter un nouveau champ](../c-about-settings-menu/c-about-metadata-menu.md#task_6DF188C0FC7F4831A4444CA9AFA615E5)de balise meta.
1. Cliquez sur **[!UICONTROL Add]**.
...
1. Sur la [!DNL Edit Ranking Rule Group] page, dans le **[!UICONTROL Rule Group Name]** champ, tapez un nom unique pour le groupe de règles.
1. Dans la liste **[!UICONTROL Rank Field Name]** déroulante, sélectionnez un nom de champ de métadonnées de classement à associer au groupe de règles. Sélectionnez **[!UICONTROL None]** si vous ne souhaitez pas affecter de rang.
...
1. Dans le menu produit, cliquez sur **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**.
1. Sur la [!DNL Define Ranking Rules] page, dans la liste **[!UICONTROL Select Rule Group]** déroulante, sélectionnez le groupe à supprimer.
...
## Vérification du classement des groupes de règles {#task_5694459D050C4254A25186038CD66B6E}
...
1. Dans le menu produit, cliquez sur **[!UICONTROL Rules]** > **[!UICONTROL Ranking Rules]** > **[!UICONTROL Edit Rules]**.
1. Sur la [!DNL Define Ranking Rules] page, dans la **[!UICONTROL Test URL]** zone, tapez l’URL d’une page Web se trouvant sur votre site Web.
...
1. Sur la [!DNL Adjust Ranking Weights] page, dans la liste **[!UICONTROL Select Rule Group]** déroulante, sélectionnez un groupe dont vous souhaitez ajuster le poids de classement.
